...
begin
    ...
    begin
        ...
        if vIndex1 = cMaxRegAdr then
            ...
            oRD1 <= regArray(vIndex1) after cRegReadTpd;
        end if;

        if vIndex2 = cMaxRegAdr then
            oRD2 <= iR15 after cRegReadTpd;
        else
            oRD2 <= regArray(vIndex2) after cRegReadTpd;
        end if;

        wait on iA1, iA2, regArray, iR15;
    end process;
    ...
    begin
        if iClk'event and iClk = '1' and iWE3 = '1' then
            ...
            if vIndex /= cMaxRegAdr then 
                regArray(vIndex) <= iWD3 after cClkToRegisterTpd;
            end if;
        end if;
        wait on iClk;
    end process;
